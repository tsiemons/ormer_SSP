...
if ($user){
    if ($removemail -like "*@*"){$user |set-aduser -remove @{proxyaddresses=$removemail}}
    if ($addmail -like "*@*"){$user |set-aduser -add @{proxyaddresses=$addmail}}
}

if ($newusername.length -lt 1){
    set-aduser -SamAccountName $newusername
}
if ($surname.length -lt 1){
    set-aduser -Surname $newusername
}
if ($newusername.length -lt 1){
    set-aduser -SamAccountName $newusername
}
